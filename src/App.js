...
export default function AdsApp() {
  ...
  const fetchAds = async () => {
    try {
      ...
      const firestoreAds = snapshot.docs.map(doc => {
        const data = doc.data();
        return {
          id: doc.id,
          title: data.title || "",
          ...
          time: data.createdAt ? "منذ قليل" : "",
          desc: data.desc || "",
          seller: data.seller || "مستخدم",
          rating: data.rating || "جديد",
        };
      });
  ...
  const handleLogin = async () => {
    if (!loginForm.email || !loginForm.password) { 
      showToast("من فضلك أدخل البيانات كاملة", "error"); 
      return; 
    }
    try {
      ...
      else if (e.code === "auth/wrong-password") errorMsg = "كلمة المرور غلط";
      else if (e.code === "auth/email-already-in-use") errorMsg = "الإيميل مسجل بالفعل";
      else if (e.code === "auth/invalid-email") errorMsg = "إيميل غير صالح";
      else if (e.code === "auth/weak-password") errorMsg = "كلمة المرور ضعيفة (٦ أحرف على الأقل)";
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      showToast("حدث خطأ في تسجيل الدخول بـ Google", "error");
    }
  };
  ...
  const handlePostAd = async () => {
    ...
    if (!postForm.title || !postForm.category || !postForm.price) { 
      showToast("من فضلك أكمل البيانات المطلوبة", "error"); 
      return; 
    }
    try {
      setLoading(true);
      const adData = {
        ...
        desc: postForm.desc,
        seller: currentUser?.displayName || currentUser?.email || "أنت",
        rating: "جديد",
      ...
      showToast("حدث خطأ في نشر الإعلان: " + e.message, "error");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div style={S.app}>
      ...
      <div style={S.topbar}>
        ...
        <div style={S.navLinks}>
          ...
          {isLoggedIn ? (
            <button style={S.btnOutline} onClick={handleLogout}>تسجيل خروج</button>
          ) : (
                ...
                <div style={S.adsGrid}>
                  {filteredAds.map(ad => (
                    <div key={ad.id} style={S.adCard} onClick={() => { setSelectedAd(ad); setPage("detail"); }}>
                      <div style={S.adImg(BG_COLORS[ad.category] || COLORS.bg)}>
                        {ad.emoji}
                      ...
                      <div style={S.adBody}>
                        ...
                        <div><span style={S.adPrice}>{ad.price}</span> <span style={S.adUnit}>{ad.unit}</span></div>
                        <div style={S.adFoot}>
                          <span style={S.adTime}>{ad.time}</span>
                          <button style={S.iconBtn} onClick={e => toggleFav(ad.id, e)}>
          ...
          {favorites.length === 0 ? (
            ...
            <div style={S.adsGrid}>
              {ads.filter(a => favorites.includes(a.id)).map(ad => (
                <div key={ad.id} style={S.adCard} onClick={() => { setSelectedAd(ad); setPage("detail"); }}>
                  <div style={S.adImg(BG_COLORS[ad.category] || COLORS.bg)}>{ad.emoji}</div>
                  <div style={S.adBody}>
                    <div style={S.adTitle}>{ad.title}</div>
                    <div style={S.adLoc}>📍 {ad.location}</div>
                    <div><span style={S.adPrice}>{ad.price}</span> <span style={S.adUnit}>{ad.unit}</span></div>
      ...
      {page === "detail" && selectedAd && (
        <div style={{ maxWidth: 800, margin: "0 auto", padding: 24 }}>
          <button style={{ ...S.btnOutline, marginBottom: 16 }} onClick={() => setPage("home")}>← رجوع</button>
          <div style={{ background: COLORS.white, borderRadius: 16, border: `1px solid ${COLORS.border}`, overflow: "hidden" }}>
            <div style={S.detailImg}>{selectedAd.emoji}</div>
            <div style={S.detailBody}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "start" }}>
                <h1 style={S.detailTitle}>{selectedAd.title}</h1>
                {selectedAd.badge && <span style={{ ...S.adBadge(selectedAd.badgeColor), position: "static" }}>{selectedAd.badge}</span>}
              </div>
              <div style={{ color: COLORS.textMuted, marginBottom: 16 }}>📍 {selectedAd.location} · {selectedAd.time}</div>
              <div style={S.detailPrice}>{selectedAd.price} {selectedAd.unit}</div>
              <div style={{ marginBottom: 20 }}>
                <h3 style={{ fontSize: 16, marginBottom: 10 }}>تفاصيل الإعلان</h3>
                <p style={{ color: COLORS.textMuted, lineHeight: 1.7 }}>{selectedAd.desc}</p>
              </div>
              <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 16 }}>
                <div style={S.detailRow}><span>الفئة</span><span>{selectedAd.category}</span></div>
                <div style={S.detailRow}><span>الموقع</span><span>{selectedAd.location}</span></div>
                <div style={S.detailRow}><span>تاريخ النشر</span><span>{selectedAd.time}</span></div>
                <div style={S.detailRow}><span>حالة الإعلان</span><span style={{ color: COLORS.success }}>متاح ✓</span></div>
              </div>
              <div style={S.sellerBox}>
                <div style={S.avatar}>{selectedAd.seller[0]}</div>
                <div>
                  <div style={{ fontWeight: 700 }}>{selectedAd.seller}</div>
                  <div style={{ fontSize: 13, color: COLORS.textMuted }}>⭐ {selectedAd.rating} — بائع موثوق</div>
                </div>
              </div>
              <div style={S.actionsRow}>
                <button style={S.btnCall}>📞 اتصل</button>
        ...
        <div style={S.modal} onClick={() => setShowLogin(false)}>
          <div style={S.modalBox} onClick={e => e.stopPropagation()}>
            <div style={S.modalHeader}>
              ...
              <button style={{ ...S.iconBtn, border: "none" }} onClick={() => setShowLogin(false)}>✕</button>
            </div>
            <div style={S.modalBody}>
              <div style={{ textAlign: "center", marginBottom: 24 }}>
                <div style={{ ...S.logoIcon, margin: "0 auto 12px" }}>⚡</div>
                <div style={{ fontSize: 16, fontWeight: 700 }}>أهلاً بك في إعلاناتي</div>
              </div>
              ...
              <button style={{ ...S.btnPrimary, width: "100%", padding: 12, marginBottom: 12 }} onClick={handleLogin}>
              ...
              <div style={{ textAlign: "center", fontSize: 13, color: COLORS.textMuted, marginBottom: 12 }}>
                {loginForm.isRegister ? (
                  <>عندك حساب؟ <span style={{ color: COLORS.primary, cursor: "pointer", fontWeight: 600 }} onClick={() => setLoginForm({...loginForm, isRegister: false})}>سجّل دخول</span></>
                ) : (
                  <>مش عندك حساب؟ <span style={{ color: COLORS.primary, cursor: "pointer", fontWeight: 600 }} onClick={() => setLoginForm({...loginForm, isRegister: true})}>سجّل دلوقتي</span></>
                )}
              </div>
              <div style={{ textAlign: "center", fontSize: 13, color: COLORS.textMuted, margin: "12px 0" }}>أو استمر بـ</div>
              <button style={{ ...S.btnOutline, width: "100%", padding: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }} onClick={handleGoogleLogin}>
                <span>🔍</span> Google
              </button>
            </div>
          </div>
        ...
        <div style={S.modal} onClick={() => setShowPostAd(false)}>
          <div style={S.modalBox} onClick={e => e.stopPropagation()}>
            <div style={S.modalHeader}>
              ...
              <button style={{ ...S.iconBtn, border: "none" }} onClick={() => setShowPostAd(false)}>✕</button>
            </div>
            <div style={S.modalBody}>
              <div style={S.uploadBox}>
                <div style={{ fontSize: 32, marginBottom: 8 }}>📷</div>
                <div style={{ fontWeight: 600 }}>أضف صور الإعلان (اختياري)</div>
                <div style={{ fontSize: 12, marginTop: 4 }}>JPG، PNG — حتى ٥ صور</div>
              </div>
              <div style={S.formGroup}>
              ...
              <button style={{ ...S.btnPrimary, width: "100%", padding: 12, opacity: loading ? 0.7 : 1 }} onClick={handlePostAd} disabled={loading}>
